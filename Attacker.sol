...
import "@openzeppelin/contracts/access/AccessControl.sol"; //Allows this control to have "roles" like an admin
import "@openzeppelin/contracts/token/ERC777/ERC777.sol"; //Allows our contract to interact with ERC777 tokens
import "@openzeppelin/contracts/token/ERC777/IERC777Recipient.sol"; //In order to receive ERC777 this contract needs to be an ERC777Recipient
import "@openzeppelin/contracts/interfaces/IERC1820Registry.sol"; //In order to receive ERC777 tokens, this contract needs to register with the EIP1820 Registry
import "./Bank.sol";
...
contract Attacker is AccessControl, IERC777Recipient {
	...
	uint8 depth = 0; //Keeps track of current depth of the recursion
	uint8 max_depth = 2; //Maximum levels of recursion in the reentrancy attack

	Bank public bank; //The bank contract that we will attack
